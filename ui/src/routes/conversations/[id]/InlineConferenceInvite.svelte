...
{#each pendingInvitations as invitation (invitation.roomId)}
  <div class="w-full px-2 pt-2 sm:px-4">
    <div
      class="mx-auto flex max-w-3xl items-center gap-3 rounded-2xl bg-secondary-500 px-3 py-3 shadow-lg ring-1 ring-white/5"
    >
      <div
        class="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-xl bg-secondary-400"
      >
        <SvgIcon icon="videoCall" moreClasses="h-5 w-5 text-tertiary-200" />
      </div>
      ...
      <div class="min-w-0 flex-1">
        ...
        <p class="truncate text-xs text-tertiary-500">
          Ongoing · {invitation.participantCount} in call
        </p>
      </div>

      <button
        on:click={() => handleJoin(invitation.roomId)}
        class="flex h-9 flex-shrink-0 items-center rounded-full bg-success-500 px-5 text-sm font-semibold text-white transition-colors hover:bg-success-600"
      >
      ...
      <button
        ...
        class="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-full text-tertiary-500 transition-colors hover:bg-secondary-400 hover:text-white"
        aria-label="Dismiss"
      >
...
>
  <div class="flex flex-col items-center gap-4 text-center">
    <div class="flex h-12 w-12 items-center justify-center rounded-full bg-warning-500/10">
      <SvgIcon icon="alertTriangle" moreClasses="h-6 w-6 text-warning-500" />
    </div>
